...
public function __construct()
	...
	public function load($id)
	{
		...
		// Cargar atributos de la base de datos
		...
		// Inicializar players como un array vacío si no se ha hecho antes
		$this->players = [];
	
		// Cargar atributos de la cuenta
		$this->attrs['accno'] = (int) $acc['id'];
		$this->attrs['password'] = (string) $acc['password'];
		$this->attrs['email'] = (string) $acc['email'];
		$this->attrs['rlname'] = isset($nicaw_acc['rlname']) ? $nicaw_acc['rlname'] : '';
		$this->attrs['location'] = isset($nicaw_acc['location']) ? $nicaw_acc['location'] : '';
		$this->attrs['comment'] = isset($nicaw_acc['comment']) ? $nicaw_acc['comment'] : '';
		$this->attrs['recovery_key'] = isset($nicaw_acc['recovery_key']) ? $nicaw_acc['recovery_key'] : '';
		$this->attrs['premium_points'] = isset($nicaw_acc['premium_points']) ? $nicaw_acc['premium_points'] : '';
	
		if (isset($acc['premdays']) && $acc['premdays'] > 0) {
			$this->attrs['premend'] = $acc['premdays'] * 24 * 3600 + time();
		} elseif (isset($acc['premEnd']) && $acc['premEnd'] > 0) {
			$this->attrs['premend'] = $acc['premEnd'];
		}
	
		// Obtener jugadores de la cuenta
...
public function getPremiumPoints()
{
    ...
    $query = 'SELECT premium_points FROM accounts WHERE id = '.$this->quote($this->attrs['accno']);
    $this->myQuery($query);
    ...
    return isset($result['premium_points']) ? (int)$result['premium_points'] : 0;
}
...
public function save()
	{
		...
		$rlname = isset($_POST['rlname']) ? $_POST['rlname'] : null;
		$location = isset($_POST['location']) ? $_POST['location'] : null;
		$comment = isset($_POST['comment']) ? $_POST['comment'] : null;
		$recovery_key = isset($_POST['recovery_key']) ? $_POST['recovery_key'] : null;
